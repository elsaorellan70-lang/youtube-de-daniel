/* ============ YouTube Personal de Daniel — app.js v8 COMPLETO ============ */
'use strict';
(() => {
  ...
  const LS = Object.freeze({
    ...
    profile: 'neontube.profile.v1', pfcache: 'neontube.pfcache.v1'
  });
  ...
  const STOP = Object.freeze(new Set(('the of and in a an en el la los las un una unos unas que y o u e de del al con por para sobre entre es son fue ser estar esta este estos estas eso esa esas ese asi mas pero sus su nos se si no lo le les me te tu yo el ella ellos muy ya cuando donde como cual cuales porque aunque sin hasta desde hacia ante tras durante mediante cada mas menos medio nueva nuevo oficial video full hd live direct en vivo trailer trailer oficial lyric lyrics lyrics video audio musica musica completa').split(/\s+/)));
  ...
  const DEMO_CATALOG = Object.freeze([
    { kind: 'file', id: 'bb', title: 'Big Buck Bunny (CC) — demo sin API key', channel: 'Blender Foundation',
      ...
      thumb: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/images/BigBuckBunny.jpg', desc: 'Cortometraje libre de Blender Foundation.', channelId: '' },
    { kind: 'file', id: 'ed', title: 'Elephants Dream (CC) — demo sin API key', channel: 'Blender Foundation',
      ...
      thumb: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/images/ElephantsDream.jpg', desc: 'Cortometraje libre de Blender Foundation.', channelId: '' },
    { kind: 'file', id: 'si', title: 'Sintel (CC) — demo sin API key', channel: 'Blender Foundation',
      ...
      thumb: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/images/Sintel.jpg', desc: 'Cortometraje libre de Blender Foundation.', channelId: '' },
    { kind: 'file', id: 'ts', title: 'Tears of Steel (CC) — demo sin API key', channel: 'Blender Foundation',
      ...
      thumb: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/images/TearsOfSteel.jpg', desc: 'Cortometraje libre de Blender Foundation.', channelId: '' }
  ]);

  const CHIPS = Object.freeze([
    { label: 'Para ti ✨',      src: { type: 'fyp' } },
  ...
  const state = {
    ...
    items: [], nextPage: null, loading: false, chId: null, fypHinted: false,
  ...
  const loadArr = (k) => { try { const a = JSON.parse(store.get(k)); return Array.isArray(a) ? a : []; } catch (_) { return []; } };
  ...
  function loadProfile() {
    try { const p = JSON.parse(store.get(LS.profile)); if (p && p.ch && p.kw) return p; } catch (_) {}
    return { ch: {}, kw: {} };
  }
  const saveProfile = (p) => store.set(LS.profile, JSON.stringify(p));
  function prune(obj, max) {
    const keys = Object.keys(obj).sort((a, b) => obj[b] - obj[a]);
    keys.slice(max).forEach(k => delete obj[k]);
  }
  function tastePush(item, bonus) {
    if (!item) return;
    const P = loadProfile();
    if (item.channelId) {
      const e = P.ch[item.channelId] || (P.ch[item.channelId] = { n: item.channel, w: 0 });
      e.n = item.channel; e.w += (bonus || 0) + 2;
    }
    String(item.title || '').toLowerCase()
      .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
      .split(/[^a-z0-9]+/)
      .filter(t => t.length > 3 && !STOP.has(t) && !/^\d+$/.test(t))
      .forEach(t => { P.kw[t] = (P.kw[t] || 0) + 1; });
    prune(P.ch, 60); prune(P.kw, 80);
    saveProfile(P);
  }
  function profileTops() {
    const P = loadProfile();
    const chs = Object.keys(P.ch).filter(id => P.ch[id].w > 0).sort((a, b) => P.ch[b].w - P.ch[a].w);
    const kws = Object.keys(P.kw).sort((a, b) => P.kw[b] - P.kw[a]);
    return { chs, kws, P };
  }
  const hasProfile = () => { const t = profileTops(); return t.chs.length > 0 || t.kws.length > 0; };
  ...
  function mapSearchItem(it) {
    ...
    return { kind: 'yt', id, title: sn.title || 'Sin título', channel: sn.channelTitle || 'Canal',
             channelId: sn.channelId || '', thumb, desc: sn.description || '', ts: sn.publishTime || '',
  ...
  function mapChartItem(it) {
    ...
    if (!thumb.startsWith('https://i.ytimg.com/')) return null;
    return { kind: 'yt', id, title: sn.title || 'Sin título', channel: sn.channelTitle || 'Canal',
             channelId: sn.channelId || '', thumb, desc: sn.description || '', ts: sn.publishedAt || '',
             dur: parseISO8601(it.contentDetails && it.contentDetails.duration) };
  }

  async function buildFYP() {
    try {
      const c = JSON.parse(store.get(LS.pfcache));
      if (c && c.items && c.ts && Date.now() - c.ts < 1800000) {
        const cached = c.items.filter(validItem);
        if (cached.length) return cached;
      }
    } catch (_) {}
    const { chs, kws } = profileTops();
    let out = [];
    for (const id of chs.slice(0, 2)) {
      try {
        const d = await ytFetch('/search', {
          part: 'snippet', type: 'video', channelId: id, order: 'date',
          maxResults: '8', videoEmbeddable: 'true', safeSearch: 'moderate'
        });
        out = out.concat((d.items || []).map(mapSearchItem).filter(Boolean));
      } catch (_) {}
    }
    if (kws.length) {
      try {
        const d = await ytFetch('/search', {
          ...
          maxResults: '12', q: kws.slice(0, 4).join(' ')
        });
        out = out.concat((d.items || []).map(mapSearchItem).filter(Boolean));
      } catch (_) {}
    }
    const seen = {};
    out = out.filter(v => { if (seen[v.id]) return false; seen[v.id] = 1; return true; });
    store.set(LS.pfcache, JSON.stringify({ ts: Date.now(), items: out.slice(0, 40) }));
    return out;
  }
  ...
  async function renderChannelView() {
    ...
    renderSkeleton(3);
    try {
      const ch = await ytFetch('/channels', { part: 'snippet,statistics,contentDetails', id: state.chId });
      const c = ch.items && ch.items[0];
      if (!c) { feed.replaceChildren(el('p', 's', 'Canal no disponible.')); return; }
      const head = el('div', 'channel-head glass');
      const back = el('button', 'icon-btn', '←');
      back.type = 'button'; back.setAttribute('aria-label', 'Volver al feed');
      back.addEventListener('click', () => { state.view = 'feed'; render(); });
      const img = el('img');
      img.src = ((c.snippet.thumbnails && (c.snippet.thumbnails.medium || c.snippet.thumbnails.default)) || {}).url || THUMB_FALLBACK;
      img.alt = '';
      const info = el('div');
      info.appendChild(el('div', 't', c.snippet.title));
      info.appendChild(el('div', 's', fmtSubs(c.statistics && c.statistics.subscriberCount) + ' suscriptores'));
      const follow = el('button', 'pill neon-outline', '★ Seguir');
      follow.type = 'button';
      follow.addEventListener('click', () => subsAdd(c.snippet.title));
      head.append(back, img, info, follow);
      feed.appendChild(head);
      const up = c.contentDetails && c.contentDetails.relatedPlaylists && c.contentDetails.relatedPlaylists.uploads;
      if (!up) { feed.appendChild(el('p', 's', 'Sin vídeos públicos.')); return; }
      const pl = await ytFetch('/playlistItems', { part: 'snippet', playlistId: up, maxResults: '20' });
      const vids = (pl.items || []).map(pi => {
        const sn = pi.snippet || {};
        const vid = sn.resourceId && sn.resourceId.videoId;
        const thumb = ((sn.thumbnails && (sn.thumbnails.medium || sn.thumbnails.default)) || {}).url || '';
        if (!YT_ID_RE.test(String(vid)) || !thumb.startsWith('https://i.ytimg.com/')) return null;
        return { kind: 'yt', id: vid, title: sn.title || 'Sin título', channel: sn.channelTitle || c.snippet.title,
                 channelId: state.chId, thumb, desc: sn.description || '', ts: sn.publishedAt || '', dur: '' };
      }).filter(Boolean);
      const frag = document.createDocumentFragment();
      vids.forEach((it, i) => frag.appendChild(cardFor(it, vids, 'feed', i)));
      feed.appendChild(frag);
      if (!vids.length) feed.appendChild(el('p', 's', 'Sin vídeos públicos.'));
    } catch (err) {
      feed.replaceChildren(el('p', 's', 'No se pudo cargar el canal (cuota/red).'));
    }
  }
  function openChannel(item) {
    if (!item || !item.channelId) { toast('Este vídeo no trae ID de canal'); return; }
    state.chId = item.channelId;
    state.view = 'channel';
  ...
  async function loadFeed(reset = true) {
    ...
    note.hidden = true;
    let src = state.query.trim() ? { type: 'search', q: state.query.trim() } : state.source;
    ...
    if (src.type === 'fyp') {
      if (hasProfile()) {
        ...
        try {
          state.items = await buildFYP();
          state.nextPage = null;
          render();
        } catch (err) { toast('No se pudo armar tu Para ti'); }
        finally { state.loading = false; $('topProgress').hidden = true; }
        return;
      }
      if (!state.fypHinted) { state.fypHinted = true; toast('✨ Tu "Para ti" aprende de lo que ves: por ahora, tendencias'); }
      src = { type: 'chart' };
    }
    if (src.type === 'favs') {
    ...
    if (src.type === 'subs') {
      ...
      try {
        let acc = [];
        for (const chName of SUBS.slice(0, 5)) {
          const data = await ytFetch('/search', {
            part: 'snippet', type: 'video', videoEmbeddable: 'true', safeSearch: 'moderate',
            maxResults: '6', q: chName, order: 'date'
          });
  ...
  function favToggle(item) {
    ...
    else { FAVS.unshift(Object.assign({}, item)); tastePush(item, 3); toast('Añadido a favoritos ❤'); }
  ...
  function cardFor(item, list, mode, index) {
    ...
    const meta = el('div', 'meta');
    const av = el('button', 'avatar-init');
    av.type = 'button';
    av.setAttribute('aria-label', 'Ver canal: ' + (item.channel || ''));
    av.textContent = (item.channel || '?').trim().charAt(0).toUpperCase();
    av.addEventListener('click', (ev) => { ev.stopPropagation(); openChannel(item); });
    meta.appendChild(av);
    const txt = el('div');
    txt.appendChild(el('div', 't', item.title));
    const chLine = el('button', 'ch-link');
    chLine.type = 'button';
    chLine.textContent = item.channel;
    chLine.addEventListener('click', (ev) => { ev.stopPropagation(); openChannel(item); });
    txt.appendChild(chLine);
  ...
  function render() {
    ...
    if (state.view === 'channel') { renderChannelView(); return; }
    const feed = $('feed');
    feed.replaceChildren();
  ...
  function bindUI() {
    ...
    $('btnPlCreate').addEventListener('click', () => { plCreate($('plNameInput').value); $('plNameInput').value = ''; });
